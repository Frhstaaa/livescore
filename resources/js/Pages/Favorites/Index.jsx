                    ...
                    {followingTeams && followingTeams.length > 0 ? (
                        followingTeams.map((team, idx) => (
                            <motion.div
                                ...
                                className="bg-white rounded-2xl p-3.5 border border-gray-100 shadow-card flex items-center justify-between cursor-pointer hover:border-brand-300 transition-all"
                            >
                                <div className="flex items-center space-x-3">
                                    <div className="w-10 h-10 rounded-full bg-slate-50 border border-gray-200 flex items-center justify-center font-bold text-xs text-slate-700 overflow-hidden shrink-0">
                                        {team.logo_url ? (
                                    ...
                                    <div>
                                        <h4 className="text-sm font-black text-gray-900 leading-tight">{team.name}</h4>
                                        <p className="text-[11px] text-gray-400 font-medium">Pelatih: {team.coach_name || 'Coach Futsal'}</p>
                                    </div>
                                </div>
                ...
                {selectedTeamModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                        <motion.div
                            ...
                            className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl relative overflow-hidden max-h-[85vh] flex flex-col"
                        >
                            {/* Close Button */}
                            <button
                            ...
                            <div className="flex items-center space-x-4 mb-4 pb-4 border-b border-gray-100">
                                <div className="w-16 h-16 rounded-2xl bg-slate-50 border border-gray-200 shadow-md flex items-center justify-center overflow-hidden shrink-0">
                                    {selectedTeamModal.logo_url ? (
                                        <img src={selectedTeamModal.logo_url} alt="" className="w-full h-full object-contain p-1" />
                                    ) : (
                                        <span className="font-black text-sm text-slate-700">{selectedTeamModal.short_name}</span>
                                    )}
                                </div>
                                <div>
                                    <span className="px-2 py-0.5 rounded-full text-[9px] font-black bg-brand-50 text-brand-600 uppercase tracking-wider mb-1 inline-block">
                                        KODE: {selectedTeamModal.short_name}
                                    </span>
                                    <h3 className="text-lg font-black text-gray-900 leading-tight">{selectedTeamModal.name}</h3>
                                    <p className="text-xs text-gray-500 font-semibold mt-0.5">Pelatih: {selectedTeamModal.coach_name || '-'}</p>
                                </div>
                            </div>

                            {/* Team Quick Stats Grid */}
                            {selectedTeamModal.standings && selectedTeamModal.standings.length > 0 && (
                                <div className="grid grid-cols-4 gap-2 mb-4 p-3 bg-gray-50 rounded-2xl text-center text-xs">
                                    <div>
                                        <span className="text-[10px] text-gray-400 font-bold block">MAIN</span>
                                        <span className="font-black text-gray-900">{selectedTeamModal.standings[0].played}</span>
                                    </div>
                                    <div>
                                        <span className="text-[10px] text-emerald-600 font-bold block">MENANG</span>
                                        <span className="font-black text-emerald-600">{selectedTeamModal.standings[0].win}</span>
                                    </div>
                                    <div>
                                        <span className="text-[10px] text-amber-600 font-bold block">SERI</span>
                                        <span className="font-black text-amber-600">{selectedTeamModal.standings[0].draw}</span>
                                    </div>
                                    <div>
                                        <span className="text-[10px] text-brand-600 font-bold block">POIN</span>
                                        <span className="font-black text-brand-600">{selectedTeamModal.standings[0].points}</span>
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="flex items-center justify-between mb-2">
                                <h4 className="text-xs font-black text-gray-400 uppercase tracking-wider flex items-center">
                            ...
                            <div className="flex-1 overflow-y-auto space-y-2 pr-1 no-scrollbar min-h-[160px]">
                                {selectedTeamModal.players && selectedTeamModal.players.length > 0 ? (
                                    selectedTeamModal.players.map((player) => (
                                        <div key={player.id} className="flex items-center justify-between p-2.5 rounded-xl bg-gray-50 text-xs border border-gray-100 hover:bg-gray-100/80 transition-colors">
                                            ...
                                            <span className="text-[10px] font-bold text-gray-400 uppercase bg-white px-2 py-0.5 rounded-md border border-gray-100">
                                                {player.position}
                                            </span>
                                        </div>
                                    ))
                                ) : (
                                    <p className="text-xs text-gray-400 text-center py-6">Belum ada pemain terdaftar di skuad ini.</p>
                                )}
                            </div>
